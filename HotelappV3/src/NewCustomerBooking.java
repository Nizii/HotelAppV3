...
public class NewCustomerBooking {
	...
	// Startet die APP
	public static void startNewCustomerBookingFrame(int id) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					list.clear();
					list = Controller.getCustomerDataWithId(id);
					NewCustomerBooking window = new NewCustomerBooking();
	...
	// Erstellt die APP
	public NewCustomerBooking() throws SQLException {
		initialize();
	}
	...
	private void initialize() {
		...
		tf_firstname = new JTextField(list.get(1));
		...
		tf_lastname = new JTextField(list.get(2));
		...
		tf_nationality = new JTextField(list.get(3));
		...
		tf_street = new JTextField(list.get(4));
		...
		tf_zip = new JTextField(list.get(5));
		...
		tf_country = new JTextField(list.get(7));
		...
		tf_email = new JTextField(list.get(8));
		...
		tf_city = new JTextField(list.get(6));
		...
		btnSaveOpenInsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (tf_firstname.getText().trim().length() == 0 || tf_firstname.getText().trim().length() == 0
					...
					try {
						BookingModel.getBookingData(0, list, null);
		...
		btnSaveOpenInsert.setBounds(10, 538, 97, 25);
		...
		btnSaveInsert.setBounds(124, 538, 97, 25);
